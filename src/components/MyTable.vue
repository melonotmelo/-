            ...
            <el-row>
                ...
                <el-col :span="10">
                    ...
                    <el-carousel-item v-for="item in img_src" :key="item">
                        <h3 class="small">
                            <img v-bind:src="item" alt="" width="100%">
...
export default {
    ...
    methods: {
        async getFeedbackList() {
            ...
            this.all_table_data = [];
            try {
                const res = await axios.get('feedback/feedbackList/', {params:{id: window.sessionStorage.getItem('id')}});
                console.log(res);
                //console.log(res);
                if(res.status !== 200) {
        ...
        async checkOperator(row) {
            ...
            try {
                ...
                for(let item of this.data4Dlg.url) {
                    this.img_src.push("http://" + item);
                }
            } catch(err) {
        ...
        async submitStar(feedbackid, star) {
            ...
            const res = await axios.post('feedback/makePoints/', {feedbackid: feedbackid, star: star});
            console.log(res);
            if(res.status !== 200) {
        ...
        async update() {
            const res = await axios.post('feedback/updateFeedback/', {
                    fedbackid: this.data4Dlg.feedbackid,
